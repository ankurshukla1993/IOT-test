...
public class PathInterpolatorCompat implements Interpolator {
    ...
    private void initQuad(float controlX, float controlY) {
        ...
        path.quadTo(controlX, controlY, FlexItem.FLEX_SHRINK_DEFAULT, FlexItem.FLEX_SHRINK_DEFAULT);
        initPath(path);
    }
    ...
    private void initCubic(float x1, float y1, float x2, float y2) {
        Path path = new Path();
        path.moveTo(0.0f, 0.0f);
        path.cubicTo(x1, y1, x2, y2, FlexItem.FLEX_SHRINK_DEFAULT, FlexItem.FLEX_SHRINK_DEFAULT);
        initPath(path);
    }
    ...
    private void initPath(Path path) {
        ...
        if (((double) Math.abs(this.mX[0])) > EPSILON || ((double) Math.abs(this.mY[0])) > EPSILON || ((double) Math.abs(this.mX[numPoints - 1] - FlexItem.FLEX_SHRINK_DEFAULT)) > EPSILON || ((double) Math.abs(this.mY[numPoints - 1] - FlexItem.FLEX_SHRINK_DEFAULT)) > EPSILON) {
    ...
    public float getInterpolation(float t) {
        ...
        if (t >= FlexItem.FLEX_SHRINK_DEFAULT) {
