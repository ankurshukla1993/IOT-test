...
public class AnimatorInflaterCompat {
    ...
    public static Animator loadAnimator(Context context, Resources resources, Theme theme, @AnimatorRes int id) throws NotFoundException {
        return loadAnimator(context, resources, theme, id, FlexItem.FLEX_SHRINK_DEFAULT);
    }
        ...
        while (true) {
            ...
            if (type != 3 && type != 1) {
                ...
                if (count > 0) {
                    ...
                    if (endFraction < FlexItem.FLEX_SHRINK_DEFAULT) {
                        if (endFraction >= 0.0f) {
                            lastKeyframe.setFraction(FlexItem.FLEX_SHRINK_DEFAULT);
                        } else {
                            keyframes.add(keyframes.size(), createNewKeyframe(lastKeyframe, FlexItem.FLEX_SHRINK_DEFAULT));
                            count++;
                        }
                    }
                    ...
                    for (i = 0; i < count; i++) {
                        ...
                        if (keyframe.getFraction() >= 0.0f) {
                            if (i == 0) {
                                ...
                                keyframe.setFraction(FlexItem.FLEX_SHRINK_DEFAULT);
                            } else {
                                int startIndex = i;
                                endIndex = i;
                                j = startIndex + 1;
    ...
    private static Keyframe loadKeyframe(Context context, Resources res, Theme theme, AttributeSet attrs, int valueType, XmlPullParser parser) throws XmlPullParserException, IOException {
        ...
        float fraction = TypedArrayUtils.getNamedFloat(a, parser, "fraction", 3, FlexItem.FLEX_BASIS_PERCENT_DEFAULT);
        TypedValue keyframeValue = TypedArrayUtils.peekNamedValue(a, parser, "value", 0);
