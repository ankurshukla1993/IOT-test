...
public final class ColorUtils {
    ...
    public static void RGBToHSL(@IntRange(from = 0, to = 255) int r, @IntRange(from = 0, to = 255) int g, @IntRange(from = 0, to = 255) int b, @NonNull float[] outHsl) {
        ...
        if (max == min) {
            ...
            s = deltaMaxMin / (FlexItem.FLEX_SHRINK_DEFAULT - Math.abs((2.0f * l) - FlexItem.FLEX_SHRINK_DEFAULT));
        }
        h = (60.0f * h) % 360.0f;
        if (h < 0.0f) {
            h += 360.0f;
        ...
        outHsl[1] = constrain(s, 0.0f, (float) FlexItem.FLEX_SHRINK_DEFAULT);
        outHsl[2] = constrain(l, 0.0f, (float) FlexItem.FLEX_SHRINK_DEFAULT);
    }
    ...
    public static int HSLToColor(@NonNull float[] hsl) {
        ...
        float s = hsl[1];
        float l = hsl[2];
        float c = (FlexItem.FLEX_SHRINK_DEFAULT - Math.abs((2.0f * l) - FlexItem.FLEX_SHRINK_DEFAULT)) * s;
        float m = l - (0.5f * c);
        float x = c * (FlexItem.FLEX_SHRINK_DEFAULT - Math.abs(((h / 60.0f) % 2.0f) - FlexItem.FLEX_SHRINK_DEFAULT));
        int r = 0;
        int g = 0;
        int b = 0;
        switch (((int) h) / 60) {
