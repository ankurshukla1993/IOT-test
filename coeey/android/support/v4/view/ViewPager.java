...
public class ViewPager extends ViewGroup {
    ...
    private static final Comparator<ItemInfo> COMPARATOR = new C03251();
    ...
    private static final Interpolator sInterpolator = new C03262();
    ...
    private final Runnable mEndScrollRunnable = new C03273();
    ...
    static class C03251 implements Comparator<ItemInfo> {
        C03251() {
        }
    ...
    static class C03262 implements Interpolator {
        C03262() {
        }
    ...
    class C03284 implements OnApplyWindowInsetsListener {
        private final Rect mTempRect = new Rect();
    ...
    void initViewPager() {
        ...
        ViewCompat.setOnApplyWindowInsetsListener(this, new C03284());
    }
    ...
    void smoothScrollTo(int x, int y, int velocity) {
        ...
        float distance = ((float) halfWidth) + (((float) halfWidth) * distanceInfluenceForSnapDuration(Math.min(FlexItem.FLEX_SHRINK_DEFAULT, (FlexItem.FLEX_SHRINK_DEFAULT * ((float) Math.abs(dx))) / ((float) width))));
        velocity = Math.abs(velocity);
        if (velocity > 0) {
            duration = Math.round(1000.0f * Math.abs(distance / ((float) velocity))) * 4;
        } else {
            duration = (int) ((FlexItem.FLEX_SHRINK_DEFAULT + (((float) Math.abs(dx)) / (((float) this.mPageMargin) + (((float) width) * this.mAdapter.getPageWidth(this.mCurItem))))) * 100.0f);
        }
    ...
    private void calculatePageOffsets(ItemInfo curItem, int curIndex, ItemInfo oldCurInfo) {
        ...
        this.mLastOffset = curItem.position == N + -1 ? (curItem.offset + curItem.widthFactor) - FlexItem.FLEX_SHRINK_DEFAULT : AutoScrollHelper.NO_MAX;
        int i = curIndex - 1;
        while (i >= 0) {
        ...
        while (i < itemCount) {
            ...
            if (ii.position == N - 1) {
                this.mLastOffset = (ii.widthFactor + offset) - FlexItem.FLEX_SHRINK_DEFAULT;
            }
            ii.offset = offset;
            offset += ii.widthFactor + marginOffset;
            i++;
    ...
    public void draw(Canvas canvas) {
        ...
        if (overScrollMode == 0 || (overScrollMode == 1 && this.mAdapter != null && this.mAdapter.getCount() > 1)) {
            ...
            if (!this.mRightEdge.isFinished()) {
                ...
                height = (getHeight() - getPaddingTop()) - getPaddingBottom();
                canvas.rotate(90.0f);
                canvas.translate((float) (-getPaddingTop()), (-(this.mLastOffset + FlexItem.FLEX_SHRINK_DEFAULT)) * ((float) width));
