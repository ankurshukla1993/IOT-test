...
public class FontResourcesParserCompat {
    ...
    private static FamilyResourceEntry readFamily(XmlPullParser parser, Resources resources) throws XmlPullParserException, IOException {
        ...
        int timeoutMs = array.getInteger(C0076R.styleable.FontFamily_fontProviderFetchTimeout, 500);
        array.recycle();
    ...
    private static FontFileResourceEntry readFont(XmlPullParser parser, Resources resources) throws XmlPullParserException, IOException {
        ...
        int weight = array.getInt(C0076R.styleable.FontFamilyFont_fontWeight, 400);
        if (1 != array.getInt(C0076R.styleable.FontFamilyFont_fontStyle, 0)) {
            isItalic = false;
        }
        int resourceId = array.getResourceId(C0076R.styleable.FontFamilyFont_font, 0);
        String filename = array.getString(C0076R.styleable.FontFamilyFont_font);
        array.recycle();
        while (parser.next() != 3) {
            skip(parser);
        }
