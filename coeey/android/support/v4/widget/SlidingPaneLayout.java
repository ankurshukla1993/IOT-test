...
public class SlidingPaneLayout extends ViewGroup {
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        if (this.mFirstLayout) {
            float f = (this.mCanSlide && this.mPreservedOpenState) ? FlexItem.FLEX_SHRINK_DEFAULT : 0.0f;
            this.mSlideOffset = f;
        }
        for (i = 0; i < childCount; i++) {
            View child = getChildAt(i);
            if (child.getVisibility() != 8) {
                ...
                if (lp.slideable) {
                    ...
                    offset = (int) ((FlexItem.FLEX_SHRINK_DEFAULT - this.mSlideOffset) * ((float) this.mParallaxBy));
                    xStart = nextXStart;
                }
                if (isLayoutRtl) {
    ...
    private boolean openPane(View pane, int initialVelocity) {
        if (!this.mFirstLayout && !smoothSlideTo(FlexItem.FLEX_SHRINK_DEFAULT, initialVelocity)) {
            return false;
        }
    ...
    public boolean isOpen() {
        return !this.mCanSlide || this.mSlideOffset == FlexItem.FLEX_SHRINK_DEFAULT;
    }
    ...
    private void dimChildView(View v, float mag, int fadeColor) {
        ...
        if (mag > 0.0f && fadeColor != 0) {
            int color = (((int) (((float) ((ViewCompat.MEASURED_STATE_MASK & fadeColor) >>> 24)) * mag)) << 24) | (16777215 & fadeColor);
            if (lp.dimPaint == null) {
    ...
    private void parallaxOtherViews(float slideOffset) {
            ...
            if ((isLayoutRtl ? slideLp.rightMargin : slideLp.leftMargin) <= 0) {
                ...
                for (i = 0; i < childCount; i++) {
                    ...
                    if (v == this.mSlideableView) {
                        int oldOffset = (int) ((FlexItem.FLEX_SHRINK_DEFAULT - this.mParallaxOffset) * ((float) this.mParallaxBy));
                        this.mParallaxOffset = slideOffset;
                        dx = oldOffset - ((int) ((FlexItem.FLEX_SHRINK_DEFAULT - slideOffset) * ((float) this.mParallaxBy)));
                        if (isLayoutRtl) {
                            dx = -dx;
                        }
                        v.offsetLeftAndRight(dx);
                        if (!dimViews) {
                            dimChildView(v, isLayoutRtl ? this.mParallaxOffset - FlexItem.FLEX_SHRINK_DEFAULT : FlexItem.FLEX_SHRINK_DEFAULT - this.mParallaxOffset, this.mCoveredFadeColor);
                        }
                    }
                }
            }
        ...
        for (i = 0; i < childCount; i++) {
            v = getChildAt(i);
            if (v == this.mSlideableView) {
                int oldOffset2 = (int) ((FlexItem.FLEX_SHRINK_DEFAULT - this.mParallaxOffset) * ((float) this.mParallaxBy));
                this.mParallaxOffset = slideOffset;
                dx = oldOffset2 - ((int) ((FlexItem.FLEX_SHRINK_DEFAULT - slideOffset) * ((float) this.mParallaxBy)));
                if (isLayoutRtl) {
                    dx = -dx;
                }
                v.offsetLeftAndRight(dx);
