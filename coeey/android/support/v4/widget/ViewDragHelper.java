...
public class ViewDragHelper {
    ...
    private static final Interpolator sInterpolator = new C03571();
    ...
    private final Runnable mSetIdleRunnable = new C03582();
    ...
    static class C03571 implements Interpolator {
        C03571() {
        }
    ...
    public static ViewDragHelper create(ViewGroup forParent, float sensitivity, Callback cb) {
        ...
        helper.mTouchSlop = (int) (((float) helper.mTouchSlop) * (FlexItem.FLEX_SHRINK_DEFAULT / sensitivity));
        return helper;
    }
    ...
    private int computeAxisDuration(int delta, int velocity, int motionRange) {
        ...
        float distance = ((float) halfWidth) + (((float) halfWidth) * distanceInfluenceForSnapDuration(Math.min(FlexItem.FLEX_SHRINK_DEFAULT, ((float) Math.abs(delta)) / ((float) width))));
        velocity = Math.abs(velocity);
        if (velocity > 0) {
            duration = Math.round(1000.0f * Math.abs(distance / ((float) velocity))) * 4;
        } else {
            duration = (int) (((((float) Math.abs(delta)) / ((float) motionRange)) + FlexItem.FLEX_SHRINK_DEFAULT) * 256.0f);
        }
