...
public class Explode extends Visibility {
    ...
    public Animator onDisappear(ViewGroup sceneRoot, View view, TransitionValues startValues, TransitionValues endValues) {
        ...
        float endX = startX;
        float endY = startY;
        int[] interruptedPosition = (int[]) startValues.view.getTag(C0171R.id.transition_position);
    ...
    private void calculateOut(View sceneRoot, Rect bounds, int[] outVector) {
        ...
        if (xVector == 0.0f && yVector == 0.0f) {
            xVector = ((float) (Math.random() * 2.0d)) - FlexItem.FLEX_SHRINK_DEFAULT;
            yVector = ((float) (Math.random() * 2.0d)) - FlexItem.FLEX_SHRINK_DEFAULT;
        }
