...
class ViewUtilsApi14 implements ViewUtilsImpl {
    ...
    public void setTransitionAlpha(@NonNull View view, float alpha) {
        Float savedAlpha = (Float) view.getTag(C0171R.id.save_non_transition_alpha);
        if (savedAlpha != null) {
    ...
    public void saveNonTransitionAlpha(@NonNull View view) {
        if (view.getTag(C0171R.id.save_non_transition_alpha) == null) {
            view.setTag(C0171R.id.save_non_transition_alpha, Float.valueOf(view.getAlpha()));
        }
    }

    public void clearNonTransitionAlpha(@NonNull View view) {
        if (view.getVisibility() == 0) {
            view.setTag(C0171R.id.save_non_transition_alpha, null);
        }
    }
    ...
    public void setAnimationMatrix(@NonNull View view, Matrix matrix) {
        ...
        float sin = values[3];
        float cos = ((float) Math.sqrt((double) (FlexItem.FLEX_SHRINK_DEFAULT - (sin * sin)))) * ((float) (values[0] < 0.0f ? -1 : 1));
