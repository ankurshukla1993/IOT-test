...
public class BottomNavigationItemView extends FrameLayout implements ItemView {
    ...
    public BottomNavigationItemView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        int inactiveLabelSize = res.getDimensionPixelSize(C0086R.dimen.design_bottom_navigation_text_size);
        int activeLabelSize = res.getDimensionPixelSize(C0086R.dimen.design_bottom_navigation_active_text_size);
        this.mDefaultMargin = res.getDimensionPixelSize(C0086R.dimen.design_bottom_navigation_margin);
        this.mShiftAmount = inactiveLabelSize - activeLabelSize;
        this.mScaleUpFactor = (((float) activeLabelSize) * FlexItem.FLEX_SHRINK_DEFAULT) / ((float) inactiveLabelSize);
        this.mScaleDownFactor = (((float) inactiveLabelSize) * FlexItem.FLEX_SHRINK_DEFAULT) / ((float) activeLabelSize);
        LayoutInflater.from(context).inflate(C0086R.layout.design_bottom_navigation_item, this, true);
        setBackgroundResource(C0086R.drawable.design_bottom_navigation_item_background);
        this.mIcon = (ImageView) findViewById(C0086R.id.icon);
        this.mSmallLabel = (TextView) findViewById(C0086R.id.smallLabel);
        this.mLargeLabel = (TextView) findViewById(C0086R.id.largeLabel);
    }
    ...
    public void setChecked(boolean checked) {
        ...
        if (this.mShiftingMode) {
            if (checked) {
                ...
                this.mLargeLabel.setScaleX(FlexItem.FLEX_SHRINK_DEFAULT);
                this.mLargeLabel.setScaleY(FlexItem.FLEX_SHRINK_DEFAULT);
            } else {
                iconParams = (LayoutParams) this.mIcon.getLayoutParams();
            ...
            this.mSmallLabel.setScaleX(FlexItem.FLEX_SHRINK_DEFAULT);
            this.mSmallLabel.setScaleY(FlexItem.FLEX_SHRINK_DEFAULT);
        }
        refreshDrawableState();
    }
