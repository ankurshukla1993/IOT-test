...
public class SnackbarContentLayout extends LinearLayout implements ContentViewCallback {
    ...
    public SnackbarContentLayout(Context context, AttributeSet attrs) {
        ...
        this.mMaxWidth = a.getDimensionPixelSize(C0086R.styleable.SnackbarLayout_android_maxWidth, -1);
        this.mMaxInlineActionWidth = a.getDimensionPixelSize(C0086R.styleable.SnackbarLayout_maxActionInlineWidth, -1);
        a.recycle();
    }

    protected void onFinishInflate() {
        super.onFinishInflate();
        this.mMessageView = (TextView) findViewById(C0086R.id.snackbar_text);
        this.mActionView = (Button) findViewById(C0086R.id.snackbar_action);
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        int multiLineVPadding = getResources().getDimensionPixelSize(C0086R.dimen.design_snackbar_padding_vertical_2lines);
        int singleLineVPadding = getResources().getDimensionPixelSize(C0086R.dimen.design_snackbar_padding_vertical);
    ...
    public void animateContentIn(int delay, int duration) {
        ...
        this.mMessageView.animate().alpha(FlexItem.FLEX_SHRINK_DEFAULT).setDuration((long) duration).setStartDelay((long) delay).start();
        if (this.mActionView.getVisibility() == 0) {
            this.mActionView.setAlpha(0.0f);
            this.mActionView.animate().alpha(FlexItem.FLEX_SHRINK_DEFAULT).setDuration((long) duration).setStartDelay((long) delay).start();
        }
    }

    public void animateContentOut(int delay, int duration) {
        this.mMessageView.setAlpha(FlexItem.FLEX_SHRINK_DEFAULT);
        this.mMessageView.animate().alpha(0.0f).setDuration((long) duration).setStartDelay((long) delay).start();
        if (this.mActionView.getVisibility() == 0) {
            this.mActionView.setAlpha(FlexItem.FLEX_SHRINK_DEFAULT);
