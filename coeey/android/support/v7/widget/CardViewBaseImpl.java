...
class CardViewBaseImpl implements CardViewImpl {
    ...
    class C04251 implements RoundRectHelper {
        C04251() {
        }

        public void drawRoundRect(Canvas canvas, RectF bounds, float cornerRadius, Paint paint) {
            float twoRadius = cornerRadius * 2.0f;
            float innerWidth = (bounds.width() - twoRadius) - FlexItem.FLEX_SHRINK_DEFAULT;
            float innerHeight = (bounds.height() - twoRadius) - FlexItem.FLEX_SHRINK_DEFAULT;
            if (cornerRadius >= FlexItem.FLEX_SHRINK_DEFAULT) {
                ...
                canvas.drawRect((bounds.left + roundedCornerRadius) - FlexItem.FLEX_SHRINK_DEFAULT, bounds.top, FlexItem.FLEX_SHRINK_DEFAULT + (bounds.right - roundedCornerRadius), bounds.top + roundedCornerRadius, paint);
                canvas.drawRect((bounds.left + roundedCornerRadius) - FlexItem.FLEX_SHRINK_DEFAULT, bounds.bottom - roundedCornerRadius, FlexItem.FLEX_SHRINK_DEFAULT + (bounds.right - roundedCornerRadius), bounds.bottom, paint);
            }
    ...
    public void initStatic() {
        RoundRectDrawableWithShadow.sRoundRectHelper = new C04251();
    }
