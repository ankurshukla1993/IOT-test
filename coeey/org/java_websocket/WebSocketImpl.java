                    ...
                    if (curop == Opcode.CLOSING) {
                        ...
                        if (curop != Opcode.CONTINUOUS) {
                            ...
                            if (this.current_continuous_frame.getOpcode() == Opcode.TEXT) {
                                off = Math.max(this.current_continuous_frame.getPayloadData().limit() - 64, 0);
                        ...
                        if (curop != Opcode.TEXT || Charsetfunctions.isValidUTF8(f.getPayloadData())) {
                            if (curop == Opcode.CONTINUOUS && this.current_continuous_frame != null && this.current_continuous_frame.getOpcode() == Opcode.TEXT) {
                                off = Math.max(this.current_continuous_frame.getPayloadData().limit() - 64, 0);
                                this.current_continuous_frame.append(f);
                                if (!Charsetfunctions.isValidUTF8(this.current_continuous_frame.getPayloadData(), off)) {
                                    throw new InvalidDataException(1007);
