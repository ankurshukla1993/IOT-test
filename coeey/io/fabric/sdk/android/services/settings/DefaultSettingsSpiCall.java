...
class DefaultSettingsSpiCall extends AbstractSpiCall implements SettingsSpiCall {
    ...
    public JSONObject invoke(SettingsRequest requestData) {
        ...
        try {
            ...
            Fabric.getLogger().mo5779d(Fabric.TAG, "Requesting settings from " + getUrl());
            Fabric.getLogger().mo5779d(Fabric.TAG, "Settings query params were: " + queryParams);
            toReturn = handleResponse(httpRequest);
            if (httpRequest != null) {
                Fabric.getLogger().mo5779d(Fabric.TAG, "Settings request ID: " + httpRequest.header(AbstractSpiCall.HEADER_REQUEST_ID));
            }
        } catch (HttpRequestException e) {
            Fabric.getLogger().mo5782e(Fabric.TAG, "Settings request failed.", e);
            toReturn = null;
            if (httpRequest != null) {
                Fabric.getLogger().mo5779d(Fabric.TAG, "Settings request ID: " + httpRequest.header(AbstractSpiCall.HEADER_REQUEST_ID));
            }
    ...
    JSONObject handleResponse(HttpRequest httpRequest) {
        ...
        Fabric.getLogger().mo5779d(Fabric.TAG, "Settings result was: " + statusCode);
        if (requestWasSuccessful(statusCode)) {
            return getJsonObjectFrom(httpRequest.body());
        }
        Fabric.getLogger().mo5781e(Fabric.TAG, "Failed to retrieve settings from " + getUrl());
        return null;
    }

    boolean requestWasSuccessful(int httpStatusCode) {
        return httpStatusCode == 200 || httpStatusCode == 201 || httpStatusCode == 202 || httpStatusCode == ScanDevice.LINK_WIFI;
    }
    ...
    private JSONObject getJsonObjectFrom(String httpRequestBody) {
        try {
            ...
            Fabric.getLogger().mo5780d(Fabric.TAG, "Failed to parse settings JSON from " + getUrl(), e);
            Fabric.getLogger().mo5779d(Fabric.TAG, "Settings response " + httpRequestBody);
            return null;
        }
    }
