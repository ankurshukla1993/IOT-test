package com.google.android.gms.common.internal;

import android.app.PendingIntent;
import android.os.Handler;
import android.os.Looper;
import android.os.Message;
import android.util.Log;
import com.google.android.gms.common.ConnectionResult;

final class zzh extends Handler {
    private /* synthetic */ zzd zzfwg;

    public zzh(zzd com_google_android_gms_common_internal_zzd, Looper looper) {
        this.zzfwg = com_google_android_gms_common_internal_zzd;
        super(looper);
    }

    private static void zza(Message message) {
        ((zzi) message.obj).unregister();
    }

    private static boolean zzb(Message message) {
        return message.what == 2 || message.what == 1 || message.what == 7;
    }

    public final void handleMessage(Message message) {
        PendingIntent pendingIntent = null;
        if (this.zzfwg.zzfwd.get() != message.arg1) {
            if (zzb(message)) {
                zza(message);
            }
        } else if ((message.what == 1 || message.what == 7 || message.what == 4 || message.what == 5) && !this.zzfwg.isConnecting()) {
            zza(message);
        } else if (message.what == 4) {
            this.zzfwg.zzfwb = new ConnectionResult(message.arg2);
            if (!this.zzfwg.zzake() || this.zzfwg.zzfwc) {
                r0 = this.zzfwg.zzfwb != null ? this.zzfwg.zzfwb : new ConnectionResult(8);
                this.zzfwg.zzfvs.zzf(r0);
                this.zzfwg.onConnectionFailed(r0);
                return;
            }
            this.zzfwg.zza(3, null);
        } else if (message.what == 5) {
            r0 = this.zzfwg.zzfwb != null ? this.zzfwg.zzfwb : new ConnectionResult(8);
            this.zzfwg.zzfvs.zzf(r0);
            this.zzfwg.onConnectionFailed(r0);
        } else if (message.what == 3) {
            if (message.obj instanceof PendingIntent) {
                pendingIntent = (PendingIntent) message.obj;
            }
            ConnectionResult connectionResult = new ConnectionResult(message.arg2, pendingIntent);
            this.zzfwg.zzfvs.zzf(connectionResult);
            this.zzfwg.onConnectionFailed(connectionResult);
        } else if (message.what == 6) {
            this.zzfwg.zza(5, null);
            if (this.zzfwg.zzfvx != null) {
                this.zzfwg.zzfvx.onConnectionSuspended(message.arg2);
            }
            this.zzfwg.onConnectionSuspended(message.arg2);
            this.zzfwg.zza(5, 1, null);
        } else if (message.what == 2 && !this.zzfwg.isConnected()) {
            zza(message);
        } else if (zzb(message)) {
            ((zzi) message.obj).zzakg();
        } else {
            Log.wtf("GmsClient", "Don't know how to handle message: " + message.what, new Exception());
        }
    }
}
