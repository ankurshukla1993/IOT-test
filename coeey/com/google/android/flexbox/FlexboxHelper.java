...
class FlexboxHelper {
    ...
    int[] createReorderedIndices(View viewBeforeAdded, int indexForViewBeforeAdded, LayoutParams paramsForViewBeforeAdded, SparseIntArray orderCache) {
        int childCount = this.mFlexContainer.getFlexItemCount();
        List<Order> orders = createOrders(childCount);
        Order orderForViewToBeAdded = new Order(null);
        ...
        while (i < childCount) {
            ...
            if (child != null) {
                if (child.getVisibility() != 8) {
                    ...
                    if (flexItem.getFlexBasisPercent() != FlexItem.FLEX_BASIS_PERCENT_DEFAULT && mainMode == Ints.MAX_POWER_OF_TWO) {
    ...
    private void checkSizeConstraints(View view, int index) {
        ...
        if (needsMeasure) {
            int widthSpec = MeasureSpec.makeMeasureSpec(childWidth, Ints.MAX_POWER_OF_TWO);
            int heightSpec = MeasureSpec.makeMeasureSpec(childHeight, Ints.MAX_POWER_OF_TWO);
                ...
                if (!(child == null || child.getVisibility() == 8)) {
                    ...
                    if (flexDirection == 0 || flexDirection == 1) {
                        ...
                        if (!this.mChildrenFrozen[index] && flexItem.getFlexGrow() > 0.0f) {
                            ...
                            childWidthMeasureSpec = MeasureSpec.makeMeasureSpec(newWidth, Ints.MAX_POWER_OF_TWO);
                        ...
                        if (!this.mChildrenFrozen[index] && flexItem.getFlexGrow() > 0.0f) {
                            ...
                            childHeightMeasureSpec = MeasureSpec.makeMeasureSpec(newHeight, Ints.MAX_POWER_OF_TWO);
                            child.measure(childWidthMeasureSpec, childHeightMeasureSpec);
                            childMeasuredWidth = child.getMeasuredWidth();
                            childMeasuredHeight = child.getMeasuredHeight();
                            updateMeasureCache(index, childWidthMeasureSpec, childHeightMeasureSpec, child);
                    ...
                    if (flexDirection == 0 || flexDirection == 1) {
                        ...
                        if (!this.mChildrenFrozen[index] && flexItem.getFlexShrink() > 0.0f) {
                            ...
                            if (newWidth < flexItem.getMinWidth()) {
                                ...
                                if (((double) accumulatedRoundError) > 1.0d) {
                                    newWidth++;
                                    accumulatedRoundError -= FlexItem.FLEX_SHRINK_DEFAULT;
                                } else if (((double) accumulatedRoundError) < -1.0d) {
                                    newWidth--;
                                    accumulatedRoundError += FlexItem.FLEX_SHRINK_DEFAULT;
                                }
                ...
                switch (this.mFlexContainer.getAlignContent()) {
                    ...
                    case 3:
                        if (totalCrossSize < size) {
                            ...
                            newFlexLines = new ArrayList();
                            int flexLineSize = flexLines.size();
                            for (i = 0; i < flexLineSize; i++) {
                                newFlexLines.add((FlexLine) flexLines.get(i));
                                if (i != flexLines.size() - 1) {
                                    ...
                                    if (accumulatedError > FlexItem.FLEX_SHRINK_DEFAULT) {
                                        dummySpaceFlexLine.mCrossSize++;
                                        accumulatedError -= FlexItem.FLEX_SHRINK_DEFAULT;
                                    } else if (accumulatedError < FlexItem.FLEX_BASIS_PERCENT_DEFAULT) {
                                        dummySpaceFlexLine.mCrossSize--;
                                        accumulatedError += FlexItem.FLEX_SHRINK_DEFAULT;
                                    }
                    ...
                    case 4:
                        ...
                        int spaceTopAndBottom = (size - totalCrossSize) / (flexLines.size() * 2);
                        newFlexLines = new ArrayList();
                        dummySpaceFlexLine = new FlexLine();
                    ...
                    case 5:
                        if (totalCrossSize < size) {
                            ...
                            for (i = 0; i < flexLinesSize; i++) {
                                flexLine2 = (FlexLine) flexLines.get(i);
    ...
    private void stretchViewVertically(View view, int crossSize, int index) {
        ...
        int childWidthSpec = MeasureSpec.makeMeasureSpec(measuredWidth, Ints.MAX_POWER_OF_TWO);
        int childHeightSpec = MeasureSpec.makeMeasureSpec(newHeight, Ints.MAX_POWER_OF_TWO);
    ...
    private void stretchViewHorizontally(View view, int crossSize, int index) {
        ...
        int childHeightSpec = MeasureSpec.makeMeasureSpec(measuredHeight, Ints.MAX_POWER_OF_TWO);
        int childWidthSpec = MeasureSpec.makeMeasureSpec(newWidth, Ints.MAX_POWER_OF_TWO);
        view.measure(childWidthSpec, childHeightSpec);
        updateMeasureCache(index, childWidthSpec, childHeightSpec, view);
        this.mFlexContainer.updateViewCache(index, view);
    }
