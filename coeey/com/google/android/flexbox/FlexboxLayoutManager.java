...
public class FlexboxLayoutManager extends LayoutManager implements FlexContainer, ScrollVectorProvider {
    ...
    public static class LayoutParams extends android.support.v7.widget.RecyclerView.LayoutParams implements FlexItem {
        public static final Creator<LayoutParams> CREATOR = new C15111();
        ...
        static class C15111 implements Creator<LayoutParams> {
            C15111() {
            }
    ...
    private static class SavedState implements Parcelable {
        public static final Creator<SavedState> CREATOR = new C15121();
        private int mAnchorOffset;
        private int mAnchorPosition;

        static class C15121 implements Creator<SavedState> {
            C15121() {
            }
    ...
    public int getSumOfCrossSize() {
        ...
        for (int i = 0; i < this.mFlexLines.size(); i++) {
            sum += ((FlexLine) this.mFlexLines.get(i)).mCrossSize;
        }
        return sum;
    }
    ...
    private int layoutFlexLineMainAxisHorizontal(FlexLine flexLine, LayoutState layoutState) {
        if ($assertionsDisabled || this.mFlexboxHelper.mMeasureSpecCache != null) {
            ...
            switch (this.mJustifyContent) {
                ...
                case 3:
                    childLeft = (float) paddingLeft;
                    spaceBetweenItem = ((float) (parentWidth - flexLine.mMainSize)) / (flexLine.mItemCount != 1 ? (float) (flexLine.mItemCount - 1) : FlexItem.FLEX_SHRINK_DEFAULT);
                    childRight = (float) (parentWidth - paddingRight);
                    break;
                case 4:
    ...
    private int layoutFlexLineMainAxisVertical(FlexLine flexLine, LayoutState layoutState) {
        if ($assertionsDisabled || this.mFlexboxHelper.mMeasureSpecCache != null) {
            ...
            switch (this.mJustifyContent) {
                ...
                case 3:
                    childTop = (float) paddingTop;
                    spaceBetweenItem = ((float) (parentHeight - flexLine.mMainSize)) / (flexLine.mItemCount != 1 ? (float) (flexLine.mItemCount - 1) : FlexItem.FLEX_SHRINK_DEFAULT);
                    childBottom = (float) (parentHeight - paddingBottom);
                    break;
                case 4:
                    if (flexLine.mItemCount != 0) {
