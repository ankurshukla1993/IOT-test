...
public class ProfilePictureView extends FrameLayout {
    ...
    class C12601 implements Callback {
        C12601() {
        }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (MeasureSpec.getMode(heightMeasureSpec) != Ints.MAX_POWER_OF_TWO && params.height == -2) {
            newHeight = getPresetSizeInPixels(true);
            heightMeasureSpec = MeasureSpec.makeMeasureSpec(newHeight, Ints.MAX_POWER_OF_TWO);
            customMeasure = true;
        }
        if (MeasureSpec.getMode(widthMeasureSpec) != Ints.MAX_POWER_OF_TWO && params.width == -2) {
    ...
    private void parseAttributes(AttributeSet attrs) {
        TypedArray a = getContext().obtainStyledAttributes(attrs, C1100R.styleable.com_facebook_profile_picture_view);
        setPresetSize(a.getInt(C1100R.styleable.com_facebook_profile_picture_view_com_facebook_preset_size, -1));
        this.isCropped = a.getBoolean(C1100R.styleable.com_facebook_profile_picture_view_com_facebook_is_cropped, true);
        a.recycle();
    }
    ...
    private void setBlankProfilePicture() {
        ...
        if (this.customizedDefaultProfilePicture == null) {
            setImageBitmap(BitmapFactory.decodeResource(getResources(), isCropped() ? C1100R.drawable.com_facebook_profile_picture_blank_square : C1100R.drawable.com_facebook_profile_picture_blank_portrait));
            return;
        }
    ...
    private void sendImageRequest(boolean allowCachedResponse) {
        ImageRequest request = new Builder(getContext(), ImageRequest.getProfilePictureUri(this.profileId, this.queryWidth, this.queryHeight)).setAllowCachedRedirects(allowCachedResponse).setCallerTag(this).setCallback(new C12601()).build();
        if (this.lastRequest != null) {
            ImageDownloader.cancelRequest(this.lastRequest);
        }
    ...
    private int getPresetSizeInPixels(boolean forcePreset) {
        ...
        switch (this.presetSizeType) {
            case LARGE /*-4*/:
                dimensionId = C1100R.dimen.com_facebook_profilepictureview_preset_size_large;
                break;
            case -3:
                dimensionId = C1100R.dimen.com_facebook_profilepictureview_preset_size_normal;
                break;
            case -2:
                dimensionId = C1100R.dimen.com_facebook_profilepictureview_preset_size_small;
                break;
            case -1:
                if (forcePreset) {
                    dimensionId = C1100R.dimen.com_facebook_profilepictureview_preset_size_normal;
