...
public class LoginButton extends FacebookButtonBase {
    ...
    class C12572 extends AccessTokenTracker {
        C12572() {
        }
    ...
    private class LoginClickListener implements OnClickListener {
        ...
        class C12591 implements DialogInterface.OnClickListener {
            C12591() {
            }
        ...
        public void onClick(View v) {
            ...
            if (accessToken == null) {
                ...
                String message;
                String logout = LoginButton.this.getResources().getString(C1100R.string.com_facebook_loginview_log_out_action);
                String cancel = LoginButton.this.getResources().getString(C1100R.string.com_facebook_loginview_cancel_action);
                Profile profile = Profile.getCurrentProfile();
                if (profile == null || profile.getName() == null) {
                    message = LoginButton.this.getResources().getString(C1100R.string.com_facebook_loginview_logged_in_using_facebook);
                } else {
                    message = String.format(LoginButton.this.getResources().getString(C1100R.string.com_facebook_loginview_logged_in_as), new Object[]{profile.getName()});
                }
                Builder builder = new Builder(context);
                builder.setMessage(message).setCancelable(true).setPositiveButton(logout, new C12591()).setNegativeButton(cancel, null);
                builder.create().show();
            } else {
                LoginButton.this.getLoginManager().logOut();
            }
    ...
    private void checkToolTipSettings() {
        switch (this.toolTipMode) {
            case AUTOMATIC:
                ...
                return;
            case DISPLAY_ALWAYS:
                displayToolTip(getResources().getString(C1100R.string.com_facebook_tooltip_default));
                return;
            default:
                return;
        }
    ...
    protected void configureButton(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        ...
        if (isInEditMode()) {
            setBackgroundColor(getResources().getColor(C1100R.color.com_facebook_blue));
            this.loginText = "Log in with Facebook";
        } else {
            this.accessTokenTracker = new C12572();
        }
        setButtonText();
    }

    protected int getDefaultStyleResource() {
        return C1100R.style.com_facebook_loginview_default_style;
    }

    private void parseLoginButtonAttributes(Context context, AttributeSet attrs, int defStyleAttr, int defStyleRes) {
        this.toolTipMode = ToolTipMode.DEFAULT;
        TypedArray a = context.getTheme().obtainStyledAttributes(attrs, C1100R.styleable.com_facebook_login_view, defStyleAttr, defStyleRes);
        try {
            this.confirmLogout = a.getBoolean(C1100R.styleable.com_facebook_login_view_com_facebook_confirm_logout, true);
            this.loginText = a.getString(C1100R.styleable.com_facebook_login_view_com_facebook_login_text);
            this.logoutText = a.getString(C1100R.styleable.com_facebook_login_view_com_facebook_logout_text);
            this.toolTipMode = ToolTipMode.fromInt(a.getInt(C1100R.styleable.com_facebook_login_view_com_facebook_tooltip_mode, ToolTipMode.DEFAULT.getValue()));
        } finally {
            a.recycle();
        }
    }
    ...
    protected void onMeasure(int widthMeasureSpec, int heightMeasureSpec) {
        ...
        if (text == null) {
            text = resources.getString(C1100R.string.com_facebook_loginview_log_in_button_long);
            logInWidth = measureButtonWidth(text);
            if (resolveSize(logInWidth, widthMeasureSpec) < logInWidth) {
                text = resources.getString(C1100R.string.com_facebook_loginview_log_in_button);
            }
        }
        logInWidth = measureButtonWidth(text);
        text = this.logoutText;
        if (text == null) {
            text = resources.getString(C1100R.string.com_facebook_loginview_log_out_button);
        }
    ...
    private void setButtonText() {
        ...
        if (AccessToken.getCurrentAccessToken() != null) {
            ...
            if (this.logoutText != null) {
                ...
                charSequence = resources.getString(C1100R.string.com_facebook_loginview_log_out_button);
            }
            setText(charSequence);
        } else if (this.loginText != null) {
            setText(this.loginText);
        } else {
            String text = resources.getString(C1100R.string.com_facebook_loginview_log_in_button_long);
            int width = getWidth();
