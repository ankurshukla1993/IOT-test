...
public class ObjectMapper {
    ...
    private <T> T _convertFromJSONObject(JSONObject jsonObject, Class<T> type) throws NoSuchMethodException, IllegalAccessException, InvocationTargetException, InstantiationException, JSONException {
        ...
        for (Field field : fields) {
            if (!Modifier.isStatic(field.getModifiers())) {
                Object setValue = getValueForField(field, jsonObject.opt(field.getName()));
                try {
    ...
    private Method getJsonValueMethod(Class<?> clazz) {
        ...
        synchronized (this.mJsonValueMethodCache) {
            method = (Method) this.mJsonValueMethodCache.get(clazz);
    ...
    private static boolean isWrapperOrPrimitiveType(Class<?> clazz) {
        if (clazz.isPrimitive() || clazz.equals(Boolean.class) || clazz.equals(Integer.class) || clazz.equals(Character.class) || clazz.equals(Byte.class) || clazz.equals(Short.class) || clazz.equals(Double.class) || clazz.equals(Long.class) || clazz.equals(Float.class)) {
            return true;
        }
        return false;
    }
