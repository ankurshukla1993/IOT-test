...
public abstract class BarLineChartBase<T extends BarLineScatterCandleBubbleData<? extends BarLineScatterCandleBubbleDataSet<? extends Entry>>> extends Chart<T> implements BarLineScatterCandleBubbleDataProvider {
    ...
    protected void init() {
        ...
        this.mBorderPaint.setStrokeWidth(Utils.convertDpToPixel(FlexItem.FLEX_SHRINK_DEFAULT));
    }
    ...
    protected void calcMinMax() {
        ...
        if (leftRange == 0.0f) {
            maxLeft += FlexItem.FLEX_SHRINK_DEFAULT;
            if (!this.mAxisLeft.isStartAtZeroEnabled()) {
                minLeft -= FlexItem.FLEX_SHRINK_DEFAULT;
            }
        }
        if (rightRange == 0.0f) {
            maxRight += FlexItem.FLEX_SHRINK_DEFAULT;
            if (!this.mAxisRight.isStartAtZeroEnabled()) {
                minRight -= FlexItem.FLEX_SHRINK_DEFAULT;
            }
        }
    ...
    protected void calcModulus() {
        if (this.mXAxis != null && this.mXAxis.isEnabled()) {
            if (!this.mXAxis.isAxisModulusCustom()) {
                ...
                this.mXAxis.mAxisLabelModulus = (int) Math.ceil((double) (((float) (((BarLineScatterCandleBubbleData) this.mData).getXValCount() * this.mXAxis.mLabelWidth)) / (this.mViewPortHandler.contentWidth() * values[0])));
            }
            if (this.mLogEnabled) {
    ...
    public float getYValueByTouchPoint(float x, float y, AxisDependency axis) {
        return (float) getValuesByTouchPoint(x, y, axis).f188y;
    }
    ...
    public int getLowestVisibleXIndex() {
        ...
        return (int) (pts[0] + FlexItem.FLEX_SHRINK_DEFAULT);
    }
